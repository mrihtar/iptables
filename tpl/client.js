  ...
  makeRuleText: function (text) {
    text = text
      ...
      .replace(/-j ([A-Z\_0-9]+)/g, function(str, name) {
        ...
        return '-j <a class="ipt-channel" href="javascript: rules.showListWithPath(\'' + name + '\', \'' + table + '\', true);">' + name + "</a>";
      });
      ...
      buttons: [
        {
          ...
          click: function() {
            ...
            rules.showListWithPath($("#chainname").val(), $("#chaintable").val());
          }
        }
      ]
    });
...
var tools = {
  ...
  initWS: function(hello) {
    if(!webSocket) {
      ...
      webSocket.onmessage = function(event) {
        var raw_data = Uint8Array.from(atob(event.data), c => c.charCodeAt(0));
        var channel = msgpack.decode(raw_data);
  ...
  showLogs: function() {
    $("#syslog").dialog({
      ...
      tools.closeWS(msgpack.encode({name: "closelog"}).toString('base64'));
    });
    tools.initWS(msgpack.encode({name: "syslog"}).toString('base64'));
  },
  ...
  showTCPDump: function() {
    $("#tcpdump").dialog({
      ...
      tools.closeWS(msgpack.encode({name: "closedump"}).toString('base64'));
      $("#dump").html("");
    });
  },
  ...
  dumpParams: function() {
    $("#dump").html("");
    tools.closeWS(msgpack.encode({name: "closedump"}).toString('base64'));
    tools.initWS(msgpack.encode({name: "dump", eth: $("#eth").val(), src: $("#src").val(), dst: $("#dst").val(), port: $("#port").val()}).toString('base64'));
  },
